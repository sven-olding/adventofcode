...
import org.example.aoc.CubeConundrum;
...
Game 5: 6 red, 1 blue, 3 green; 2 blue, 1 red, 2 green
    ...
    private static List<List<Map<String, Integer>>> testDataLarge;
    private static List<List<Map<String, Integer>>> testDataSmall;

    @BeforeAll
    public static void setupTestData() {
        testDataLarge =
                Arrays.stream(LARGE_INPUT.split("\n")).map(CubeConundrumTest::parseLine2).toList();

        testDataSmall =
                Arrays.stream(SMALL_INPUT.split("\n")).map(CubeConundrumTest::parseLine2).toList();
    }

    private static List<Map<String, Integer>> parseLine(String line) {
        var result = new ArrayList<Map<String, Integer>>();
        String[] subsets = line.split(":")[1].split(";");
        for (String subset : subsets) {
            Map<String, Integer> currentDraw = new HashMap<>();
            for (String draw : subset.split(",")) {
                String[] split = draw.trim().split(" ");
                int numOfCubes = Integer.parseInt(split[0]);
                String color = split[1];
                currentDraw.put(color, numOfCubes);
            }
            result.add(currentDraw);
        }
        return result;
    }

    private static List<Map<String, Integer>> parseLine2(String line) {
    ...
    @Test
    void testCubeConundrum_largeInput() {
        assertEquals(1931, CubeConundrum.getSumOfMatchingGameIds(testDataLarge, 12, 13, 14));
    }

    @Test
    void testCubeConundrum_smallInput() {
        assertEquals(8, CubeConundrum.getSumOfMatchingGameIds(testDataSmall, 12, 13, 14));
    }
}
